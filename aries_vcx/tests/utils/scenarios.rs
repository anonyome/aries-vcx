...
pub mod test_utils {
    ...
    pub async fn revoke_credential_and_publish_accumulator(faber: &mut Faber, issuer_credential: &Issuer, rev_reg_id: &str) {
        ...
        publish_local_revocations(faber.wallet_handle, &faber.config_issuer.institution_did, &rev_reg_id).await;
    }

    pub async fn revoke_credential_local(faber: &mut Faber, issuer_credential: &Issuer, rev_reg_id: &str) {
        let (_, delta, timestamp) = libindy::utils::anoncreds::get_rev_reg_delta_json(&rev_reg_id, None, None)
            .await
            .unwrap();
