...
pub async fn publish_schema(submitter_did: &str, wallet_handle: WalletHandle, schema: &str) -> VcxResult<()> {
    ...
    let response = sign_and_submit_to_ledger(wallet_handle, submitter_did, &request).await?;
...
pub async fn publish_cred_def(wallet_handle: WalletHandle, issuer_did: &str, cred_def_json: &str) -> VcxResult<()> {
    ...
    let response = sign_and_submit_to_ledger(wallet_handle, issuer_did, &cred_def_req).await?;
...
pub async fn publish_rev_reg_def(
    ...
    let response = sign_and_submit_to_ledger(wallet_handle, issuer_did, &rev_reg_def_req).await?;
    _check_response(&response)?;
    Ok(())
}
...
pub async fn publish_rev_reg_delta(
    ...
    let response = sign_and_submit_to_ledger(wallet_handle, issuer_did, &request).await?;
    _check_response(&response)?;
    Ok(response)
}
...
pub async fn publish_local_revocations(wallet_handle: WalletHandle, submitter_did: &str, rev_reg_id: &str) -> VcxResult<()> {
...
pub mod test_utils {
    ...
    pub async fn create_and_write_test_schema(wallet_handle: WalletHandle, submitter_did: &str, attr_list: &str) -> (String, String) {
        ...
        let response = sign_and_submit_to_ledger(wallet_handle, submitter_did, &req).await.unwrap();
...
pub mod integration_tests {
    ...
    async fn test_revoke_credential() {
        ...
        publish_local_revocations(setup.wallet_handle, &setup.institution_did, &rev_reg_id)
            .await
            .unwrap();
